...
export default function TelemetryChart({
  ...
  const activeSecondaryTelemetry = comparisonEnabled ? secondaryTelemetry : undefined
  const xValues = [
    ...telemetry.map((sample) => sample.distance),
    ...(activeSecondaryTelemetry?.map((sample) => sample.distance) ?? []),
  ]
  const yValues = [
    ...telemetry.map((sample) => getChannelValue(sample, channel)),
    ...(activeSecondaryTelemetry?.map((sample) => getChannelValue(sample, channel)) ?? []),
  ]
  const xScale = d3.scaleLinear().domain(getDomain(xValues)).range([0, plotWidth])
  const yScale = d3.scaleLinear().domain(getDomain(yValues)).range([plotHeight, 0])
  const line = d3
    .line<TelemetryModel>()
  ...
  const handlePointerMove = (event: PointerEvent<SVGSVGElement>) => {
    ...
    const targetDistance = xScale.invert(pointerX - MARGIN.left)
    const nextIndex = distanceBisector(telemetry, targetDistance)

    if (nextIndex === 0) {
      onTelemetryIndexChange(0)
      return
    }
    if (nextIndex === telemetry.length) {
      onTelemetryIndexChange(telemetry.length - 1)
      return
    }
    ...
    onTelemetryIndexChange(
      targetDistance - previousSample.distance <= nextSample.distance - targetDistance
        ? nextIndex - 1
        : nextIndex,
    )
  }
  ...
  return (
    <div ref={containerRef} className="h-[280px] w-full" aria-label={`${CHANNEL_LABELS[channel]} by distance`}>
      {canRenderChart && (
        <svg viewBox={`0 0 ${width} ${CHART_HEIGHT}`} role="img" aria-label={`${CHANNEL_LABELS[channel]} plotted against distance`} className="h-full w-full" onPointerMove={handlePointerMove}>
          <g transform={`translate(${MARGIN.left},${MARGIN.top})`}>
            ...
            {activeSecondaryTelemetry && <path d={line(activeSecondaryTelemetry) ?? undefined} fill="none" stroke="var(--color-text-muted)" strokeWidth="1.25" strokeDasharray="5 4" strokeOpacity="0.72" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke" />}
            <path d={line(telemetry) ?? undefined} fill="none" stroke="var(--color-primary-purple)" strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke" />
            {selectedSample && <line x1={xScale(selectedSample.distance)} x2={xScale(selectedSample.distance)} y1={0} y2={plotHeight} stroke="var(--color-primary-purple)" strokeWidth="1" vectorEffect="non-scaling-stroke" />}
